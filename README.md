...
## 초기 모델 설계가 중요하다는 점
- 이유
    - 처음 진행한 iOS 프로젝트로 DB 가 어떤 것이 있는지 몰랐던 상태였고
    - 해당 토이 프로젝트는 CoreData 로 이미지 & 텍스트 데이터를 저장했지만, 대부분의 다른 프로젝트들은
    - 이미지는 document 에 저장하고 관리를 JSON 파일로 하는 방식이 대부분이였습니다.
- 결과
    - 앱의 동작에는 문제가 없지만, 앞으로 다른 프로젝트를 진행할 때
    - 좋은 구조, 확장성을 생각하며 앱을 설계해야 겠다는 생각을 하게 되었습니다.
...
- 이유
    ...
    - 사진 선택 후 별도의 그룹 선택 화면이 필요했습니다.
    - 직접 라이브러리를 수정하려 했지만, 좋은 방식이 아니라고 생각했습니다.
- 결과
    - 디버깅을 통해 제공되는 picker 가 Navigation Controller 를 상속 받음을 알게되었고,
    - 따라서 필요한 그룹 선택 VC 를 Push 하여 필요한 앱 동작 구현을 할 수 있었습니다.
    - 앱을 만들면서 화면 전환에 대한 이해가 한층 높아졌다고 생각됩니다.
...
- 이유
    ...
    - 단순히 옮겼다는 생각이 들었고, MVVM 패턴을 잘 이해하고 있지 않다는 생각을 하게되었습니다.
    - best practice 를 찾으려 했지만 일관된 코드를 찾기 어려웠습니다.
- 결과
    - MVVM 모델은 표준화된 틀이 없기 때문에 패턴에 이해도에 따라 코드가 차이난다는 것을 알게되었고
    - 그에 따라 ReactorKit, RIBs ... 같은 아키텍처가 등장했다고 생각했습니다.
    - Swift, UIKit 과 더불어 아키텍처에 대한 학습도 이어나가야 겠다는 목표를 세우게 되었습니다.
